...
//                    firstChildrenBean.setChildren(secondChilds);
    ...
    public static void setPinyin(FirstChildrenBean bean) {
        final String DefaultLetter = "#";
        String letter = DefaultLetter;
        if (!TextUtils.isEmpty(bean.getName())) {
            letter = Pinyin.toPinyin(bean.getName().toCharArray()[0]);
            bean.setPinyin(letter.toUpperCase().substring(0, 1));
            if (isNumeric(bean.getPinyin()) || !check(bean.getPinyin())) {
                bean.setPinyin("#");
            }
            return;
        }
        if (letter == DefaultLetter && !TextUtils.isEmpty(bean.getName())) {
            letter = Pinyin.toPinyin(bean.getName().toCharArray()[0]);
        }
        bean.setPinyin(letter.substring(0, 1));
        if (isNumeric(bean.getPinyin()) || !check(bean.getPinyin())) {
            bean.setPinyin("#");
        }
    }
