...
public class MediaActivity extends AppCompatActivity implements View.OnClickListener, ViewPager.OnPageChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        maxTime = getIntent().getIntExtra("maxTime", 30);
        ...
        if (VideoImageList.size() > 0) {
            videoThumbnail = FileUtils.createVideoThumbnailFile(new File(VideoImageList.get(0).path));
        }
    ...
    private void listenerContainer() {
        ...
        mGridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, final int i, long l) {
                Log.i(TAG, "onItemClick: " + itemImageList.get(i).path);
                Log.i(TAG, "onItemClick: " + itemImageList.get(i).duration);

                count = selecImageList.size();

                if (type.equals(ImageModel.TYPE_IMAGE) && !ImageUtils.getEffective(itemImageList.get(i).path)) {
                ...
                if (type.equals(ImageModel.TYPE_VIDEO)) {
                    ...
                    if (ImgSize <= 0) {
                        ...
                        ToastUtils.showShort(getApplication(), "视频时长" + maxTime + "以内");
                        return;
                    }
                }
...
//                    }
                ...
                //多选图片
                if (count >= maxstr && !itemImageList.get(i).getIsChecked()) {
//                    ToastUtils.showShort(getApplication(), "你最多只能选择" + maxstr + "个图片");
                    ToastUtils.showShort(getApplication(), "你最多只能选择" + maxstr + "个" + (type.equals(ImageModel.TYPE_IMAGE) ? "图片" : "视频"));
                } else if (count >= maxstr && itemImageList.get(i).getIsChecked()) {
                    count = count - 1;
                ...
                if (count > 0) {
                    confirm.setText("发送(" + count + "/" + maxstr + ")");
    ...
    private void sendMedias() {
        try {
            ...
            intent.putExtra("video", sendList.get(0).getPath());
            intent.putExtra("cover", sendList.get(0).getThumb());
    ...
    public class SelecImageListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
            ...
            Log.d(TAG, "onBindViewHolder: isChecked " + imageModel.isChecked);
    ...
    private class GalleryPageAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            video.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    FileUtils.openFile(MediaActivity.this, new File(imageModel.getPath()));
                }
            });
...
//        });
        ...
        Log.d(TAG, "removePreviewView: " + selecImageList.size());
        if (selecImageList.size() > 0) {
            if (current > selecImageList.size()) {
                current = 0;
            }
            confirm.setText("发送(" + selecImageList.size() + "/" + maxstr + ")");
